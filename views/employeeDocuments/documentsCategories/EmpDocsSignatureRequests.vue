<template>
  ...
  <section id="employee-documents">
    <div class="inner-navbar">
      <!-- <employeeDocumentsStats></employeeDocumentsStats> -->
      <!-- Employee Documents -->
      <div class="main-content-section">
        <!-- <div class="tabs-group">
          <router-link
            class="el-button el-button--medium ml-15"
            to="/emp-documents"
            >All</router-link
          >
          <router-link
            class="el-button el-button--medium ml-15"
            to="/emp-documents/pending"
            >Pending Docs</router-link
          >
          <router-link
            class="el-button el-button--medium ml-15"
            to="/emp-documents/expired"
            >Expired Docs</router-link
          >
          <router-link
            class="el-button active el-button--medium ml-15"
            to="/emp-documents/signature-requests"
            >Signature Requests</router-link
          >
          <router-link
            class="el-button el-button--medium ml-15"
            to="/emp-documents/waiting-for-others"
            >Waiting For Others</router-link
          >
        </div> -->
        <div class="inner-navbar-content">
          <div class="vue-data-table-default">
            <!-- <el-table :data="employeeDocumentsTableData" style="width: 100%"> -->
            <data-tables-server
              v-loading="loading"
              :fullscreen="true"
              :data="data"
            ...
            >
              ...
              <el-table-column label="E-Signature">
                <template slot-scope="scope">
                  <div v-if="scope.row.e_signature && scope.row.e_signature_date">
                    <el-tag type="success" size="small"> &#8226; Done </el-tag>
                    <div class="usr-email medium black-coral line-height-2">
                      Expired on {{ getDateWithFormat(scope.row.expires_at) }}
                    </div>
                  </div>
                  <div v-else>
                    ...
                    <div class="usr-email medium black-coral line-height-2">
                      Dued on {{ getDateWithFormat(scope.row.expires_at) }}
                    </div>
                  </div>
                </template>
              </el-table-column>
  ...
  methods: {
    ...
    async loadData(data) {
      if (this.data && this.data.length) {
        ...
        this.page = data.page;
        this.$router.push({name: 'emp-docs-signature-requests', query: { page: this.page }}).catch(()=>{})
        await this.fetchEmployeeDocuments();
      }
    },
    viewDoc(doc) {
