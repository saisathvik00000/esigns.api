        ...
        <div class="inner-navbar-content">
          <div class="vue-data-table-default" v-loading="loading">
            ...
            >
              ...
              <el-table-column label="Last Date">
                <template slot-scope="scope">
                  <span class="black-coral">{{ getDateWithFormat(scope.row.expires_at) }}</span>
                </template>
              </el-table-column>
  ...
  methods: {
    ...
    async loadData(data) {
      if (this.data && this.data.length) {
        ...
        this.page = data.page;
        this.$router.push({name: 'emp-pending-documents', query: { page: this.page }}).catch(()=>{})
        await this.fetchEmployeeDocuments();
      }
    },
    viewDoc(doc) {
