                ...
                <template slot-scope="scope">
                  <span>
                    <el-tag type="success" size="small"
                      ><i class="el-icon-success"></i> {{getDocumentSignedCount(scope.row)[0]}}
                    </el-tag>
                  </span>
                </template>
              </el-table-column>
                ...
                <template slot-scope="scope">
                  <span>
                    <el-tag type="danger" size="small"
                      ><i class="el-icon-error"></i> {{getDocumentSignedCount(scope.row)[1]}}
                    </el-tag>
                  </span>
                </template>
              </el-table-column>
  ...
  methods: {
    ...
    async loadData(data) {
      if (this.data && this.data.length) {
        ...
        this.page = data.page;
        this.$router.push({name: 'emp-docs-waiting-for-others', query: { page: this.page }}).catch(()=>{})
        await this.fetchEmployeeDocuments();
      }
    },
    viewDoc(doc) {
    ...
    getDocumentSignedCount(data) {
      if (data && data.document_users) {
        let success = 0
        let pending = 0
        data.document_users.forEach(element => {
          if (element.e_signature_required && element.e_signature_status) {
            success = success + 1
          } else {
            pending = pending + 1
          }
    ...
    getDocumentSignedPendingCount(data) {
      if (data && data.document_users) {
        let count = 0
        data.document_users.forEach(element => {
          if (element.e_signature_required && !element.e_signature_status) {
