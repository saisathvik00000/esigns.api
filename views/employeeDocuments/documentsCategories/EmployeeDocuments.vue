        ...
        <div class="inner-navbar-content">
          <div class="vue-data-table-default" v-loading="loading">
            <data-tables-server
              ...
              style="width: 100%"
            >
              <el-table-column label="Name" width="330">
              ...
              <el-table-column label="Status" width="200">
                <template slot-scope="scope">
                  <span class="rich-black">{{ scope.row.document_status }}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column label="Last Modified">
                <template slot-scope="scope">
                  <span class="usr-name d-block large black-coral">{{
                    getDateWithFormat(scope.row.updated_at)
                  }}</span>
                  <span class="usr-email medium lavendar-grey">{{
                    scope.row.last_modified_by
                  }}</span>
                </template>
              </el-table-column>
  ...
  methods: {
    ...
    async loadData(data) {
      if (this.data && this.data.length) {
        ...
        this.page = data.page;
        this.$router.push({name: 'all-documents', query: { page: this.page }}).catch(()=>{})
        await this.fetchEmployeeDocuments();
      }
    },
    viewDoc(doc) {
